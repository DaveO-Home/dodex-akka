...
/** @param brokerUrl
  ...
  * @param stashTimeToLive
  *   messages received when disconnected will be stash. Messages isOverdue
  *   after stashTimeToLive will be discard. See also `stashCapacity`
  * @param stashCapacity
  *   pubSubStash will be drop first haft elems when reach this size
  * @param reconnectDelayMin
  *   when received Disconnected event, we will first delay reconnectDelayMin to
  *   try Connect. + if connect success => we reinit connectCount + else =>
  *   ConnListener.onFailure will send Disconnected to this FSM => we
  *   re-schedule Connect with {{{delay = reconnectDelayMin * 2^connectCount}}}
...
/** Convenient class to generate MqttConnectOptions
  ...
  *   and reconnects
  * @param maxInflight
  *   The max inflight limits to how many messages (include qos 0, 1, 2) we can
  *   send without receiving acknowledgments. Default is `MAX_INFLIGHT_DEFAULT *
  *   10`
  * @param maxInflightQos12
  *   maxInflight for Publish with qos > 0. In mqttv5, see
  *   [[https://docs.oasis-open.org/mqtt/mqtt/v5.0/os/mqtt-v5.0-os.html#_Toc3901251 4.9 Flow Control]].
  *   When using mqttv3 client, server may still hardcode this setting, ex with
  *   mosquitto:
  *   [[https://mosquitto.org/man/mosquitto-conf-5.html max_inflight_messages]].
  *   TODO in mqttv5, we need update this number when receiving CONNACK
  * @param will
  *   A last will and testament message (and topic, and qos) that will be set on
  *   the connection
  */
